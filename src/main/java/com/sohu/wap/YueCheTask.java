...
public class YueCheTask  extends YueChe implements Callable<Integer> {
	...
	public YueCheTask(YueCheItem xueYuan, String date,Host proxyHost){
    ...
    private  void  doYuche () throws InterruptedException {
        ...
        do {
            ...
            if(Constants.KM3.equals(yueCheItem.getKm())){
                ret =  yuche(date, shiDuan,Constants.KM3_HiddenKM, yueCheItem.getPhoneNum());
            }else if (Constants.KM1.equals(yueCheItem.getKm())){
                ret =  yuche(date, shiDuan,Constants.KM1_HiddenKM, yueCheItem.getPhoneNum());
            }else if (Constants.KM_AUTO.equals(yueCheItem.getKm())) {
                ret =  yuche(date, shiDuan,0, yueCheItem.getPhoneNum());
            }else{
                ret =  yuche(date, shiDuan,Constants.KM2_HiddenKM, yueCheItem.getPhoneNum());
            }

            String uinfo = yueCheItem.getUserName() +":"+date+ YueCheHelper.AMPM.get(shiDuan);

            int  result  = ret.getRet();

            if (result == YueChe.BOOK_CAR_SUCCESS){
                isSuccess = true;
                uinfo = yueCheItem.getUserName() +":"+ret.getData()+":"+date+ YueCheHelper.AMPM.get(shiDuan);
